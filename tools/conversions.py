...
def graph2nx(G):
    """
    Convert a `gunfolds` graph to NetworkX format
...
def encode_sccs(g):
    ...
    s += ':- edge1(X,Y), scc(X, K), scc(Y, L), K != L, sccsize(L, Z), Z > 1, not dag(K, L). '
    s += ':- directed(X,Y,U), scc(X, K), scc(Y, L), K != L, sccsize(L, Z), Z > 1, not dag(K, L), u(U, _). '
    return s
